...
async def create_user_account(email: str, name: str) -> str:
    ...
    async with get_db_session() as session:
        ...
        existing = await session.fetchrow(
...
async def create_demo_team(users):
    ...
    async with get_db_session() as session:
        ...
        await session.execute(
            ...
            RETURNING id
            """,
            team_id, team_name, "Aeonia Team", "Main Aeonia development team", jason_id
        )
    ...
    # Add team members
    await rbac_manager.add_team_member(actual_team_id, jason_id, "admin", "system")
    await rbac_manager.add_team_member(actual_team_id, alice_id, "member", "system")
