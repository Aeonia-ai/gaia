...
async def get_nearby_locations(
    ...
    """
    Get AR waypoints near a GPS location from KB content.
    ...
    try:
        ...
        if not (-90 <= center_lat <= 90):
            raise HTTPException(
                status_code=400,
                detail=f"Latitude must be between -90 and 90"
            )
        if not (-180 <= center_lng <= 180):
            raise HTTPException(
                status_code=400,
                detail=f"Longitude must be between -180 and 180"
            )
        ...
        logger.info(
            ...
            f"({center_lat}, {center_lng}) within {radius}m"
        )
