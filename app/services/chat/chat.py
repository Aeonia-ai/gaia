...
# Import KB-enhanced multiagent orchestrator
try:
    from .kb_multiagent_orchestrator import (
        kb_enhanced_multiagent_endpoint,
        kb_research_scenario_endpoint,
        kb_gamemaster_scenario_endpoint,
        kb_development_scenario_endpoint,
        kb_search_endpoint,
        kb_context_loader_endpoint,
        kb_multi_task_endpoint
    )
    
    @router.post("/kb-enhanced")
    async def kb_enhanced_multiagent_chat(
        request: ChatRequest,
        auth_principal: Dict[str, Any] = Depends(get_current_auth)
    ):
        """
        ...
        """
        return await kb_enhanced_multiagent_endpoint(request, "adaptive", auth_principal)
    
    @router.post("/kb-research")
    async def kb_research_chat(
        request: ChatRequest,
        auth_principal: Dict[str, Any] = Depends(get_current_auth)
    ):
        """KB research with specialized knowledge agents"""
        return await kb_research_scenario_endpoint(request, auth_principal)
    
    @router.post("/kb-gamemaster")
    async def kb_gamemaster_chat(
        request: ChatRequest,
        auth_principal: Dict[str, Any] = Depends(get_current_auth)
    ):
        """Game mastering with KB-powered world knowledge"""
        return await kb_gamemaster_scenario_endpoint(request, auth_principal)
    
    @router.post("/kb-development")
    async def kb_development_chat(
        request: ChatRequest,
        auth_principal: Dict[str, Any] = Depends(get_current_auth)
    ):
        """Development guidance using KB codebase knowledge"""
        return await kb_development_scenario_endpoint(request, auth_principal)
    
    @router.post("/kb-search")
    async def kb_search_chat(
        request: ChatRequest,
        auth_principal: Dict[str, Any] = Depends(get_current_auth)
    ):
    ...
    @router.post("/kb-multitask")
    async def kb_multitask_chat(
        request: ChatRequest,
        auth_principal: Dict[str, Any] = Depends(get_current_auth)
    ):
    ...
    logger.info("✅ KB-enhanced multiagent endpoints added")
    
except Exception as e:
    logger.warning(f"⚠️ Could not import KB-enhanced multiagent: {e}")
    import traceback
    logger.warning(f"⚠️ KB Multiagent Traceback: {traceback.format_exc()}")
