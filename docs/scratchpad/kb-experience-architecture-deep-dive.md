...
```
User: "take bottle"
  ↓ ~1.5s - Pass 1 (Logic with Claude Sonnet, temp=0.2)
  ↓ ~2.0s - Pass 2 (Narrative with Claude Sonnet, temp=0.7)
  ↓ ~3.5s TOTAL (projected based on typical LLM API response times)
Response arrives
...
**Projected Impact**: Perceived latency multi-second → sub-100ms *(architecture-based)*
...
```
User: "take bottle"
  ↓ ~0.1s - world_update SSE event (bottle disappears visually)
           Based on NATS pub/sub architecture: low-latency messaging
  ↓ ~3.4s - Narrative SSE event ("You pick up the bottle...")
...
**Projected Impact**: Actual latency ~3.5s → ~2.0s (for simple commands) *(estimate)*
